...
# Asegúrate de que transaction_date sea timestamp
df = df_transactions \
...
df_mensual_usuario = df.groupBy("client_id", "mes_inicio") \
    .agg(
        F.count("*").alias("num_transacciones"),
...
windowSpec = Window.partitionBy("merchant_category").orderBy(F.col("gasto_total").desc())

df_por_comercio = df.groupBy("merchant_name", "merchant_category") \
...
df_ranking.filter(F.col("rank_en_categoria") <= 3) \
    .orderBy("merchant_category", "rank_en_categoria") \
...
windowSpec_usuario = Window.partitionBy("client_id").orderBy("transaction_date")
...
df.filter(F.col("amount") > F.col("gasto_anterior") * 2) \
  .select("client_id", "transaction_date", "amount", "gasto_anterior", "variacion_vs_anterior") \
...
windowSpec_acum = Window.partitionBy("client_id") \
    .orderBy("transaction_date") \
    .rowsBetween(Window.unboundedPreceding, Window.currentRow)

df = df.withColumn("gasto_acumulado", F.sum("amount").over(windowSpec_acum))

df.select("client_id", "transaction_date", "amount", "gasto_acumulado") \
  .filter(F.col("client_id") == df.limit(1).select("client_id").collect()[0][0]) \
  .show(10)
```
...
windowSpec_rolling = Window.partitionBy("client_id") \
    .orderBy("transaction_date") \
