...
**Semana:** 05  
**Tema:** Ingesta incremental automática + calidad de datos declarativa en DLT  
**Nivel:** Intermedio–Avanzado  
**Modalidad:** Individual  
**Entorno:** Databricks Enterprise  
**Prerequisito:** Actividad 01 completada — primer pipeline DLT funcionando
...
El segundo problema: ¿qué pasa cuando el archivo tiene una fila con `amount = null`, o un `user_id` que no existe? En semana 02–03 usabas filtros `WHERE is_null(amount)` mezclados con la lógica de negocio. **Quality Expectations** en DLT separan la calidad del código: declaras las reglas, DLT las aplica y registra cuántas filas fallaron — sin mezclar nada.
...
- ¿Cuáles son los tres comportamientos de `@dlt.expect`? (`warn`, `drop`, `fail`)
- ¿Qué es una tabla de cuarentena y cuándo se usa?
- ¿Qué columna interna agrega DLT para marcar filas que fallaron expectativas? (`_rescued_data`)
...
Crea un notebook DLT `bronze_autoloader_<tu-nombre>.py`:
...
|-----------|---------------|---------------|
| `@dlt.expect` | Registra la violación como métrica pero **no elimina** la fila | Calidad informativa (quieres saber pero no rechazar) |
| `@dlt.expect_or_drop` | **Elimina las filas** que violan la regla | Filas inválidas que contaminarían Silver |
| `@dlt.expect_or_fail` | **Detiene el pipeline** si alguna fila viola la regla | Violaciones críticas (ej: columna clave nunca puede ser null) |

```python
@dlt.expect("amount_no_nulo", "amount IS NOT NULL")
@dlt.expect_or_drop("user_id_valido", "client_id IS NOT NULL AND client_id > 0")
@dlt.expect_or_fail("fecha_requerida", "date IS NOT NULL")
@dlt.table(
    name="bronze_transactions_validada",
    comment="Transacciones con reglas de calidad aplicadas",
)
def bronze_transactions_validada():
    return dlt.read_stream("bronze_transactions_stream")
```

> El orden de los decoradores importa: primero las expectations, luego `@dlt.table`. DLT los aplica de arriba hacia abajo.
...
def silver_transactions_clean():
    ...
    return (
        dlt.read_stream("bronze_transactions_validada")
...
def get_bronze_con_flag():
    ...
    return (
        dlt.read_stream("bronze_transactions_stream")
...
@dlt.table(name="silver_transactions_validas")
...
> El quarantine pattern no usa `@dlt.expect_or_drop` porque quieres las filas — solo en otro lado. En producción, el equipo de calidad revisa la tabla de cuarentena periódicamente.
...
2. `@dlt.expect` no elimina filas pero registra la violación. ¿Tiene sentido usar `@dlt.expect` en una tabla Silver (supuestamente limpia)? ¿Cuándo sí y cuándo no?

3. La tabla de cuarentena y la tabla válida leen el mismo stream. ¿Eso significa que los datos se procesan dos veces? ¿Cómo optimiza DLT internamente este patrón?
...
| `@dlt.expect_all_or_drop` con 4 reglas | Nombres descriptivos de negocio | 15% |
...
- [DLT Data Quality — Expectations](https://docs.databricks.com/en/delta-live-tables/expectations.html)
- [Quarantine pattern in DLT](https://docs.databricks.com/en/delta-live-tables/tutorial-python.html)
